...
class BlockValidator {
  ...
  async validateBlock(block) {
    ...
    // 3
    if (
      this.bc.currentBlock.header.state.getNonce() + 1 !==
      block.header.state.getNonce()
    ) {
    ...
    if (potentialHashList.length) {
      const filteredPotential = potentialHashList.filter(hash =>
        this.pdb.isExist(hash, bc.address)
      );
      const promises = filteredPotential.forEach(hash =>
        this.db.readBlock(hash)
      );
      const result = await Promise.all(promises);
      potentialSum = result.reduce(
        (prev, curr) => prev + sumReceiveTx(curr, bc.address),
        potentialSum
      );
    }
    // 7
    if (
      bc.currentBlock.state.getBalance() + potentialSum - txValueSum !==
      block.state.getBalance()
    ) {
...
function sumReceiveTx(block, addr) {
  return block.transactions.reduce(
    (prev, curr) => (curr.receiver === addr ? prev + curr.value : prev),
    0
  );
}
 ...
 * @param {*} opAddr
 */
const validateCheckpoint = function(bc, checkpoint, opAddr) {
  if (!checkpoint.validate(opAddr)) return { error: true };
  if (checkpoint.address !== bc.address) return { error: true };
